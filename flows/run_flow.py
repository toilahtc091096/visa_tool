...
from constants import (
    ...
    L_30_PASSPORT_EMPTY_PAGES_OUTPUT_PATH,
    L_30_BANK_STATEMENT_OUTPUT_PATH,
    ...
    L_30_UNDER_18_DOCUMENTS_OUTPUT_PATH,
    L_30_AUTHORIZATION_LETTER_OUTPUT_PATH,
    L_30_HOTEL_INFO,
    PASSPORT_FILE_FOLDER,
)
...
async def run_flow(
    ...
    async with httpx.AsyncClient() as client:
        ...
        step = "get ocr"
        if (
            PASSPORT_FILE_FOLDER.get(visa_type) is None
            or PASSPORT_FILE_FOLDER.get(visa_type) == ""
        ):
            print("no passport file")
            return
        passport_file_path = get_passport_file_path(PASSPORT_FILE_FOLDER.get(visa_type))
        ...
        ok, meta = await api_passport_ocr(
            ...
            token=token,
            tmp_secret=tmp_secret,
            file_path=get_passport_file_path(PASSPORT_FILE_FOLDER.get(visa_type)),
